...
export const resetImages: (keyof typeof Images)[] =
    ['RESET_BUTTON', 'RESET_BUTTON_HOVER',
     'RESET_BUTTON_PRESSED'] as (keyof typeof Images)[];
...
export class Button {
  ...
  draw(context: CanvasRenderingContext2D) {
    const drawImage = (key: number) => context.drawImage(
        getLoadedImage(this.images[key]), this.position.x, this.position.y,
...
export class ButtonSet {
  ...
  mousedown(point: Point): boolean {
    return this.buttons.reduce((handled: boolean, element) => {
      if (handled) {
        return true;
      }
