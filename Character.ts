...
import {Direction, Point} from './xyTuple';
...
function drawCharacter(
    context: CanvasRenderingContext2D, x: number, y: number, headOffset: number,
    step: number, holdingKey: boolean, direction: Direction,
...
class CharacterAfterImage implements Sprite {
  ...
  draw(context: CanvasRenderingContext2D) {
    ...
    drawCharacter(
        context, this.position.x, this.position.y, this.headOffset,
        WALKING_STEPS[Math.floor(this.frame / CHARACTER_WALKING_CONSTANT)],
        this.holdingKey, this.direction, size);
    context.restore();
...
export class Character implements Sprite {
    ...
    if (this.velocity.length > 6) {
      if (this.afterImageCooldown <= 0) {
        ...
        return [new CharacterAfterImage(
            this.position.clone(), this.frame, this.headOffset,
            this.velocity.direction, this.holding)];
      }
    }
  ...
  draw(context: CanvasRenderingContext2D) {
    ...
    drawCharacter(
        context, this.position.x, this.position.y, this.headOffset,
        WALKING_STEPS[Math.floor(this.frame / CHARACTER_WALKING_CONSTANT)],
        this.holding, this.velocity.direction);
  }
}